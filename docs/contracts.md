...
- `configs/soc/autolink/tiles.yaml` owns the native Conv → ReLU → Pool tiled graph and its two live buffer slots.
...
`CgraConvRocketConfig` enables native Gemmini `loop_conv` and uses `gci.yaml`. `conv_i8.c` verifies a 3x3 convolution with padding, compact 2x2x8 INT8 output and automatic CGRA ReLU. Gemmini reads preloaded input from its shared SPM, reads weights from DRAM and writes results back to its SPM through the local DMA path. This is a single-output-tile test, not cross-IP tile overlap.
...
- With native `loop_conv` enabled, `gemmini_conv_begin` captures an origin-zero Conv template. The peripheral binding module extracts its full-image input geometry and replaces tile dimensions, clipped padding and input/output pointers during replay; CPU commands, weights, bias, scale and activation remain captured native values. This path requires self-contained execute/store setup, batch-one NHWC, full channels/kernel, no native pooling or training transforms, and fixed native A/B halves. It checks native field/storage limits and the exact armed publication range before replay. An incoming copy supplies a compact source view with global origin and valid shape; native input dimensions, pixel stride and pointer are rebound to that view. The view must contain the native read footprint, including its extra stride-minus-one extent. Copy acceptance records the view, not completed physical reads; the producer slot remains live until the tile drains.
...
- AutoLink snapshots an entire run before emitting row-major tile identities with clipped tails. `communication.buffer_slots` bounds live tiles; slot `tile.id % bufferSlots` is reused only after every stage releases that tile. Dependency queues preserve order, joins match the original tile context, and one owner at a time controls each physical endpoint. CPU-rooted stages rearm independently, allowing different IPs to process different tiles. Per-stage software results summarize the run at its final tile and retain the first failure.
...
- Root `EMITTING` describes cursor emission; `RUNNING` remains set until all tile slots drain. `CYCLES` counts fabric run cycles and `OVERLAP` counts cycles with active task intervals on different IPs and different tile IDs. This measures task overlap, not arithmetic-unit utilization. MMIO staging writes do not change the active run snapshot.
