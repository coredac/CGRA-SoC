...
from openfpga_demo_config import CELL_LIBRARY_FILES, DemoConfig, ensure_file
from openfpga_flow_runner import GeneratedPaths
from openfpga_pinmap import PinMap
...
def sync_rtl(demo: DemoConfig, paths: GeneratedPaths, pin_map: PinMap) -> None:
    ...
    _write_wrapper(demo.wrapper_path, demo, pin_map, fpga_ports)
...
def _write_manifest(path: Path, demo: DemoConfig, includes: List[str]) -> None:
    ...
    lines = [
        "// Auto-generated by scripts/openfpga_rtl_sync.py.",
...
def _write_wrapper(path: Path, demo: DemoConfig, pin_map: PinMap, fpga_ports: Dict[str, PortDecl]) -> None:
    cfg = demo.architecture.config_protocol
    input_reg = demo.user_interface.input_register
    output_reg = demo.user_interface.output_register
    ...
    lines = [
        ...
        f"  input [{demo.input_width - 1}:0] user_input,",
        f"  output [{demo.output_width - 1}:0] user_output",
        ");",
        "",
