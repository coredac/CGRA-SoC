...
def write_c_header(demo: DemoConfig, bitstream: PackedBitstream) -> None:
    ...
    lines = [
        ...
        f"#define {prefix}_INPUT_WIDTH {demo.input_width}",
        f"#define {prefix}_OUTPUT_WIDTH {demo.output_width}",
        "",
    ]
    lines.extend(_field_macros(prefix, "INPUT", demo.user_interface.input_register.fields))
    lines.extend(_field_macros(prefix, "OUTPUT", demo.user_interface.output_register.fields))
    lines.extend(
        [
...
def write_pin_map_header(demo: DemoConfig, pin_map: PinMap) -> None:
    prefix = demo.macro_prefix
    lines = [
    ...
    for field in demo.user_interface.input_register.fields:
    ...
    for field in demo.user_interface.output_register.fields:
...
def write_run_env(demo: DemoConfig) -> None:
    env_path = demo.bitstream_header_path.parent / "openfpga_demo.env"
    default_test = demo.name.replace("_", "-")
    lines = [
        "# Auto-generated by scripts/openfpga_test_codegen.py.",
        f"OPENFPGA_CHIPYARD_CONFIG={demo.chipyard.config_name}",
        f"OPENFPGA_DEFAULT_TEST={default_test}",
        "",
    ]
