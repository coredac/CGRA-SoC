...
def optional_bool(
    data: Mapping[str, object], key: str, default: bool, path: Path
) -> bool:
...
def make_id_to_2d_map(
    num_cgra_columns: int, num_cgra_rows: int
) -> dict[int, list[int]]:
    return {
...
def make_controller_addr_map(
    data_mem_size_global: int, num_cgra_columns: int, num_cgra_rows: int
) -> dict[int, list[int]]:
    num_cgras = num_cgra_columns * num_cgra_rows
    per_cgra_data_size = data_mem_size_global // num_cgras
    return {
        cgra_id: [cgra_id * per_cgra_data_size, (cgra_id + 1) * per_cgra_data_size - 1]
...
def translate_dut(
    dut: IntegratedCgraWithDmaRTL, output: Path, top_module: str = TOP_MODULE
) -> None:
    ...
    dut.cgra.set_metadata(VerilogTranslationPass.explicit_module_name, CGRA_MODULE)
    ...
    translated = re.sub(r" object at 0x[0-9a-fA-F]+>", " object>", translated)
    output.write_text(translated, encoding="utf-8")


def translate(
    arch_yaml: Path, soc_yaml: Path, output: Path, top_module: str = TOP_MODULE
) -> None:
