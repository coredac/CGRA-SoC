...
#define CGRA_DMA_DESC_CONST(spm_addr, nbytes, tag)                             \
  ((cgra_dma_desc_t)(CGRA_DMA_CONST_ASSERT(__builtin_constant_p(spm_addr)) +   \
                     ...
                     CGRA_DMA_CONST_ASSERT(                                    \
                         ((uint64_t)(nbytes) % CGRA_DMA_BEAT_BYTES) == 0) +    \
                     CGRA_DMA_CONST_ASSERT(                                    \
                         (uint64_t)(tag) <                                     \
...
#define CGRA_DMA_ISSUE(funct, dram, desc)                                      \
  do {                                                                         \
    ...
    asm volatile(".insn r 0x0b, 3, " CGRA_DMA_STRINGIFY(                       \
                     funct) ", x0, %0, %1\n\t" ::"r"(cgra_dma_dram_),          \
                 "r"(cgra_dma_desc_)                                           \
                 : "memory");                                                  \
  } while (0)
...
static inline uint8_t cgra_dma_wait(uint8_t expected_tag) {
  ...
  uintptr_t observed;
  asm volatile(".insn r 0x0b, 6, " CGRA_DMA_STRINGIFY(
                   CGRA_FUNCT_DMA_WAIT) ", %0, %1, x0\n\t"
               : "=r"(observed)
               : "r"(expected)
               : "memory");
