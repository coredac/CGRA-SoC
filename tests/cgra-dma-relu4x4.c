...
static const cgra_dma_desc_t MVIN_DESC = CGRA_DMA_DESC_CONST(RELU4X4_SPM_WORD_ADDR, sizeof(input), RELU4X4_MVIN_TAG);
static const cgra_dma_desc_t MVOUT_DESC = CGRA_DMA_DESC_CONST(RELU4X4_SPM_WORD_ADDR, sizeof(output), RELU4X4_MVOUT_TAG);
...
int main(void) {
  ...
  if (wait_result != 1 || complete != 1 || complete_count != RELU4X4_EXPECTED_COMPLETES || result != RELU4X4_EXPECTED_RESULT) {
